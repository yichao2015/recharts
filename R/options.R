...
#'
#' }
addGeoCoord <- function(chart, geoCoord=NULL){
    stopifnot(inherits(chart, 'echarts'))
    if (is.null(geoCoord)) return(chart)
    if (is.list(geoCoord)){
        if ('timeline' %in% names(chart$x)){
            if (chart$x$options[[1]]$series[[1]]$type == 'map')
                chart$x$options[[1]]$series[['geoCoord']] <- geoCoord
        }else{
            if (chart$x$series[[1]]$type == 'map')
                chart$x$series[['geoCoord']] <- geoCoord
        }
    }else if (is.data.frame(geoCoord)){
        if (ncol(geoCoord) < 3) stop('geoCoord must contain place, lng, lat!')

        lstGeoCoord <- unname(apply(geoCoord, 1, function(row){
